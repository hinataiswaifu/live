...
bool MapManager::checkCollision(Point position) 
{
    ...
    for(int i = 0; i < 4; i++) {
        ...
        if(hitbox[i]->x >= m_map_width || hitbox[i]->y >= m_map_height || hitbox[i]->x <0 || hitbox[i]->y <0) {
            collision = true;
            break;
        }
...
// Overloaded checkCollision applies an x and y movement to the input position before checking for collision
...
Point MapManager::tileCoordForPosition(Point position)
{
    ...
    int y = ((m_tile_map->getMapSize().height * m_tile_map->getTileSize().height) - position.y) / m_tile_map->getTileSize().height;
    return Point(x, y);
}